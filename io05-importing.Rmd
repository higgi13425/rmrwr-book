...
write_csv(scurvy, 'temp_data/scurvy.csv')
strep_tb <- medicaldata::strep_tb
write_tsv(strep_tb, 'temp_data/strep_tb.tsv')
```
...
Let's start with a file in the `temp_data` folder named `scurvy.csv`.

```{r}
read_csv('temp_data/scurvy.csv')
```
...
This is done 'automagically', as the read_csv() function reads the first 1000 rows, then guesses what data type is present in each column.
It is often conservative, and made all of these columns into character variables (also called strings).
...
```{r}
scurvy_data <- read_csv('temp_data/scurvy.csv')
```
...
This is not a file written to disk, but it is now available in the working environment.
You can now print this out at any time by typing `scurvy_data` into the Console, or into a script.
...
Once you have assigned your data to an object, it will stick around for later use.

csv is a very common data format, and most data management programs have a way to export \*.csv files.
...
Both of these functions have reasonable defaults, so that most of the time, you just have to use the path to your file as the only argument.
...
```{r}
read_csv('temp_data/scurvy.csv', col_names = c("pat_id", "arm", "dose", "gums", "skin", "weak", "lass", "fit"))
```
...
```{r}
read_csv(file = 'temp_data/scurvy.csv', 
...
```{r}
scurvy_cols <- read_csv('temp_data/scurvy.csv',
         col_types = cols(
...
You can choose - col_integer - col_character - col_number() (handles \#s with commas) - col_double (to specify decimal \#s) - col_logical (only TRUE and FALSE) - col_date(format = "") - may need to define format - col_time(format = "") - col_datetime(format = "") - col_factor(levels = "", ordered = TRUE) - may want to set levels and ordered if ordinal.
The read_csv() guesses may be fine, but you can take more control if needed.
- col_guess() - is the default - col_skip() if you want to skip a column

This gives you fine control of each column.
...
```{r}
scurvy_cols2 <- read_csv('temp_data/scurvy.csv',
         col_types = "ifcffff")
...
The file `strep_tb.tsv` is located in the `temp_data` folder.
The path to this file is `temp_data/strep_tb.csv`.
...
strep_tb_cols <- read_---('temp_data/-----.tsv',
...
The glimpse() function provides identical output.
...
It is actually much easier to see the full width and height of a data set by scrolling, which you can do when you *View()* a dataset in the RStudio viewer.
...
```{r, error=TRUE}
read_excel(path= 'temp_data/data_workbook.xlsx',
           sheet = "raw_data",
           skip = 4)
