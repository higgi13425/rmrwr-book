...
paulolol <- tempfile(fileext = ".xlsx")
req <- httr::GET(github_link, 
          # write result to disk
           write_disk(path = paulolol))
...
```{r, error=TRUE}
read_excel(paulolol, # path = "paulolol.xlsx" has been saved to paulolol
           sheet = 1,
           skip = 1)
...
`r webex::mcq(c("jump", 'sheet', answer = 'skip', 'path'))`

- which argument in read_excel lets you pick which spreadsheet tab to read? 

`r webex::mcq(c("pick", answer = 'sheet', 'skip', 'path'))`
...
`r webex::mcq(c("B2:F8", answer = 'A4:F11', 'A1:L30', 'B4:K15'))`
