...
url_stem <- "https://raw.githubusercontent.com/higgi13425/rmrwr-book/master/data/"
```
...
Let's start with a file in folder on GitHub named `scurvy.csv`. You will need to glue together the url_stem and "scurvy.csv" to get the full web address. Run the code chunk below to see the url_stem and the dataset.

```{r}
url_stem
read_csv(glue(url_stem, 'scurvy.csv'))
```
...
```{r}
scurvy_data <- read_csv(glue(url_stem, 'scurvy.csv'))
```
...
```{r}
read_csv(file = glue(url_stem, 'scurvy.csv'), 
...
1. There is a file named "paulolol.xlsx" in the GitHub repository, and you can read it in using the same url_stem and glue() to produce the filename. A picture of this problematic file is shown below.
```{r, echo=FALSE, out.width=50%}
knitr::include_graphics(glue(url_stem, "paulolol.png"))
```
