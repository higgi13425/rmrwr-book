...
- X must always be mapped before Y in ggplot2 `aes()` functions `r torf(FALSE)`

- The `medicaldata::polyps` line of code represents the `r mcq(c('geom', answer = 'data', 'aesthetic'))` layer of the ggplot.

- The `aes()` function is the aesthetic layer, which `r mcq(c('pipes', 'glues', answer = 'maps'))` data variables to visual properties (aesthetics) of the plot.
...
- Which one of the following is NOT a visual property that you can map a variable to in a scatterplot  `r mcq(c('fill',  'color', 'shape', 'alpha', answer = 'data', 'size'))`

- All mapping of variables to visual properties must be within the parentheses of the `r mcq(c('ggplot',  'geom_point', 'color', answer = 'aes', 'scale'))` function.

- You should assign visual properties to constant values, like `color = "blue"` or `size = 2` within `ggplot()` or the geom_point() function, but **outside** of an `aes()` function `r torf(answer = TRUE)`.
