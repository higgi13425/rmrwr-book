...
Now let's replicate what Will Landau (author of {targets}, working at Eli Lilly) just narrated. First, go to your
RStudio and open a New Project. Call it 'airquality_demo'. Then,
...
Now, within the R folder, create a new R script (New File is just to the right of the New Folder button in the Files tab) called 'functions.R' and
...
RStudio) the code chunk below. This will load the packages and
functions, write the data.csv file, then run each function to get and
process the data, then show the plot in your Plots tab.
...
Check your Files tab to confirm that `_targets.R` now exists in the root
of your project. Open this file to see the initial code that was
generated. This is where you will build up your pipeline. 

Within the `_targets.R` file, scroll down to the `tar_option_set()` function. This function sets the options for the pipeline, including the packages that you will need. For this pipeline, we will only need "tibble". For more complex pipelines, we will need more packages.

Now scroll down to the `list()` function with two tar_targets inside of it. 

This code will create four targets for the pipeline.
...
Notice that each `tar_target()` function has a first argumemt that is the target, or output object. The second argument is the function that generates the output object (or the datafile if we are just reading in data). The argument to each of these functions is the input object for that function, which is usually generated in the preceding `tar_target()` function.
...
``` r
list(
  tar_target(file, format = "file", pattern = "data.csv"),
...
Check that all the parnetheses and commas are correct, with no syntax errors, then save `_targets.R`

Now we are ready to start building the pipeline.
Let's start with some pre-build checks.
First, run `tar_manifest` locally in your project as shown below. This will check the pipeline and show you the status of each target. 


``` r
tar_manifest(fields = command)
```
This should generate a table in your console with two columns, showing the name of each target, and the command that generates that target. If you see any errors, you will need to fix these before proceeding.

Then we can visualize our pipeline by running the code chunk below. This will generate a graph of the pipeline, showing the targets and the dependencies between them. The graph should show the four targets, with the file target at the left, and the plot target at the far right. The data target depends on the file target, the model target depends on the data target, and the plot target depends on the model and data targets. There should be a continuous flow from left to right, though it can branch or unite in more complex workflows. As this is a new pipeline, each target will be color coded (a blue-green) as "Outdated", as each target has not been built yet.
...
This will run the pipeline and create the outputs in the `_targets` folder. Check that these are present, and view each target object with `tar_read()`.

Check the output of `tar_visnetwork()` to see that the pipeline structure is now completely up to date.
...
Use the code below (adding conditional formatting) to edit the `make_table()` function in the `functions.R` file in the R folder of the Project. Look carefully at the `tab_style()` function to see how the conditional formatting is applied.
...
Check out the appearance of the newly-modified table with `tar_read()`.

## Modify the Plot

Now modify the plot to make years with points greater than 3 million cases of TB colored red. Use the code below to edit the `plot_data()` function in the `functions.R` file in the R folder of the Project.
...
Check out the appearance of the newly-modified plot with `tar_read()`
