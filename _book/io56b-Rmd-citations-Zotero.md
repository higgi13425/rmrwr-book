...
When the Zotero Connector is installed and active, there should be a new icon at the top right of your browser window, to the right of the URL. This is a rectangle with a folded upper right corner, which looks a bit like a dog-eared page of a book. If you hover over this, the pop-up text should read, "Save to Zotero".
...
Flip back to Pubmed in the browser, and search for other helpful references, and add them to your collection. Aim for at least 5 distinct references (for now) that we can add to an rmarkdown document.
...
::: tip
Tip - you can export your MyBibliography from your Pubmed NCBI page in RIS or BibTeX format, and then import these into your My Publications collection in Zotero to have these handy.
:::
...
When you have RStudio open, start a new project, with <br>
File/New Version Control/Git. 

Enter this URL: <br>
`https://github.com/higgi13425/rmd-cite.git`.

Name the project `rmd-cite`.
Create the project.
...
In the left sidebar, select Zotero, then the particular library that you created.
...
You can click on the reference to see how it will be formatted in your bibliography.

This all works because of a few settings in the YAML header at the top of the document. The key one is `bibliography: references.bib`.

This tells Rmarkdown where to look for the references for this document, and Zotero by default puts the references in this file.
...
You can download new `*.csl` files for the journals you frequently publish in from teh Zotero Style Repository [here](https://www.zotero.org/styles).

Just search for the desired Journal title, and download the `*.csl` file. The `*.csl` file should be stored in same working directory as your rmarkdown file. 

When this is the case, you can change the bibiography style from the default by adding a line to the YAML header, just below the `bibliography: references.bib` line. Add `csl: the-new-england-journal-of-medicine.csl` to set the format to the NEJM, or `csl: gastroenterology.csl` to set to the format of the journal *Gastroenterology*. Both of these `*.csl` files should already be in your project directory.
...
The first time you try to add a citation or a bibliography, Zotero will ask you which journal to use as the default format. It has several examples available as defaults, but you will probably want to add formatting for journals specific to your specialty.

If this is the case, click on the "Manage Styles" link below and to the right of the list of Citation Styles. This will bring up the Style Manager window shown below. Click on the "Get Additional Styles" button below the list of Styles.
...
You can change the formatting of the references to a different style by clicking on Document Preferences in the Zotero menu.
