...
Change the label from "Number of bins:" to "Select a Textbook:" 
Delete the min, max, and value arguments.
...
For example, you might use `numericInput(inputID = "age", label = "Type in the age", min =1, max = 100, value = 35) The same variables will be referred to in the server section as`input$age` or `input$creatinine\`.
...
## Building a Shiny App for a Model
