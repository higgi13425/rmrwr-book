...
Theoph <- Theoph[,1:6]
ggplot(aes(x=Time, y=new$predicted, color=Subject), data=Theoph) + geom_line() +geom_point(aes(x=Time, y=conc))
```
...
```{r cor}
ggplot(aes(x=conc, y=predicted), data = Theoph) + geom_point()
