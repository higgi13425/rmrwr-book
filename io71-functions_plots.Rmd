...
covid <- medicaldata::covid_testing %>% 
  filter(!patient_class %in% c("admit after surgery-ip", "admit after surgery-obs", "day surgery")) %>% 
  filter(ct_result<41) %>% 
  filter(age <100) %>% 
  filter(!is.na(patient_class)) %>% 
  filter(patient_class != "not applicable")

prostate <- medicaldata::blood_storage %>% janitor::clean_names() %>% 
  filter(!is.na(b_gs)) %>% 
  mutate(bgs = factor(b_gs))

infert <- medicaldata::infert %>% janitor::clean_names() %>% 
...
```{r prostate-plot-1, warning=FALSE}
rbc_age_group = 1
...
```{r prostate-plot-2, warning=FALSE}
rbc_age_group = 1
...
```{r prostate-plot-3, error=TRUE, eval=FALSE}
rbc_age_group = 1
...
```{r prostate-plot-4, error=TRUE, eval=FALSE}
rbc_age_group = 1:3
...
```{r prostate-plot-5, error=TRUE, eval=FALSE}
rbc_age_group = 1
...
```{r prostate-plot-6, error=TRUE, eval=FALSE}
prostate %>% 
  nest(data = -var) %>% 
...
```{r prostate-plot-7, error=TRUE, eval=FALSE}
prostate %>% 
...
```{r prostate-plot-8, error=TRUE, eval=FALSE}
prostate %>% 
  nest(data = -rbc_age_group) %>% 
