...
```{r pipe-hunt, exercise = TRUE, error=TRUE}
mtcars %>% 
  filter(cyl >4) %>% 
  select(mpg, hp, displ) %>% 
...
```{r pipe-fix, exercise = TRUE, error = TRUE}
iris %>% 
...
```{r plot-debug, exercise = TRUE, echo=FALSE, error=TRUE}
iris %>% 
...
## Pipe 2
```{r pipe-debug-2, exercise = TRUE, error = TRUE}
starwars %>% 
  filter(height <180) %>% 
...
## Plot 2
```{r plot-debug-2, exercise = TRUE, error=TRUE}
murders %>% 
...
## Plot3
```{r plot-debug-3, exercise = TRUE, error=TRUE}
...
## Pipe 3
```{r pipe-debug-3, exercise=TRUE, error = TRUE}
gapminder %>% 
  filter(year == 1965) %>% 
