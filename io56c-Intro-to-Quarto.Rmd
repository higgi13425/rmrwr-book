...
For example, you can write out "my two-sided p value is `` `r 2*(1-pnorm(7, mean = 5, sd = 1.1))` ``," which shows the code as written. When this R code between the backticks is evaluated (when you render the document), the resulting p value is shown. This will read: <br> My two-sided p value is `r 2*(1-pnorm(7, mean = 5, sd = 1.1))`.
